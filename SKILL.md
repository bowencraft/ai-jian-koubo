...
> 多轨素材编辑 + 火山引擎转录 + AI 口误识别 + 网页审核 / 字幕格式化

## 模式

- **模式 A: 剪口播 / 剪播客** — 人类拼多轨素材 → 合成审核音频 → 转录 → 口误识别 → 网页审核 → 多轨 FCPXML / 音频 / SRT 导出
...
0. 确认视频路径/素材路径 + 选择模式
1. 模式 A 先打开素材编辑页：人类创建/调整多轨 project.json
...
请选择模式：
  [A] 剪口播 — 识别口误 → 网页审核 → 导出 FCPXML / 音频 / SRT
...
```json
{
  ...
  "name": "episode-demo",
  "assets": [
    { "id": "host", "name": "Host Mic", "path": "/path/to/host.m4a", "kind": "audio", "hasAudio": true, "hasVideo": false },
    { "id": "guest", "name": "Guest Mic", "path": "/path/to/guest.m4a", "kind": "audio", "hasAudio": true, "hasVideo": false }
  ],
  "clips": [
    { "id": "host-1", "assetId": "host", "timelineStart": 0, "sourceStart": 0, "duration": 600, "trackIndex": 0, "lane": 0, "audioRole": "dialogue.host" },
    { "id": "guest-1", "assetId": "guest", "timelineStart": 0, "sourceStart": 0, "duration": 600, "trackIndex": 1, "lane": 1, "audioRole": "dialogue.guest" }
  ]
}
```
...
bash "$SKILL_DIR/scripts/run_multitrack_transcribe.sh" "$PROJECT_FILE" "$BASE_DIR"
...
用户在网页中：播放片段确认 → 勾选/取消 → 点击「导出为...」→ 选择 FCPXML、剪好音频 MP3（96/128/192/256 kbps）或剪后 SRT。生成的 `*_cut.fcpxml` 可拖入剪映或 Final Cut Pro 完成最终剪辑。

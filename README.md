...
一个**剪口播/剪播客的 coding-agent skill**:人类先拼接多轨素材 → 合成审核音频 → 外部 API 转文字 → AI 识别口误/口癖/静音 → 网页波形审核 → 导出引用原始素材的 FCPXML,也可导出剪好的 MP3 音频和 SRT 字幕。核心是一份 [`SKILL.md`](SKILL.md),**任何支持 skill、能读文件、能跑 shell 的 coding agent 都能装来用**,不绑定特定工具。

![审核页界面](assets/review-ui.png)

> 新流程:先在「素材编辑」页把主持人、嘉宾、BGM、片头片尾等素材放到多轨时间线；再转文字和 AI 预剪；审核页能看到多轨摘要,但只负责词级删改。改到满意点「导出为...」,选择 FCPXML、剪好音频或 SRT 字幕。
...
- **附加导出音频 / 字幕** — 同一份审核结果可导出剪好的 MP3(96/128/192/256 kbps)和重排时间轴后的 SRT
...
6. 满意后点 **「导出为...」**；主线选择 FCPXML,也可选择剪好的 MP3 音频或 SRT 字幕
...
页面布局参考剪辑软件:左上角素材库,右上角 16:9 预览,左下片段属性,右下多轨时间线。新增素材通过上传按钮加入当前项目,素材路径保存为项目目录相对路径；把素材卡片拖到下方轨道即可新增片段,也可以导入/导出项目 JSON。预览时视频取当前时间最高的视频轨,音频轨全部同步播放。

...
| 素材编辑 | 无 | 素材库、拖放、多轨移动/修剪、真实波形、缩放、禁用与 Solo |
| 审核时间线 | 单波形 | clip 内上半波形；叠加模式 / 独立模式；正常、删减、气口三态 |
| 审核播放 | 单一旧素材 | 时间线变化后重建多轨审核混音，避免播放旧音频 |
| 工程导出 | 单素材 FCPXML | 多 `<asset>`、多轨 `asset-clip`，按原始素材 source range 裁切 |
| 附加导出 | FCPXML | FCPXML、MP3（96–256 kbps）、剪后 SRT |
| 人机交接 | 依赖对话说明 | 编辑完成和审核导出后都有可复制提示词，引导 agent 创建后续任务 |
| 审核包 | 基础目录 | 收集全部引用素材、改写相对路径，同时生成便携目录和 ZIP |
| 交换文件体积 | 无专门缓存 | 真实 waveform 存本地 `waveform_cache.json`，不进入项目 JSON / 审核包 JSON |
...
原项目由栗氪聊AI创建；本 fork 保留原项目许可证与归属，并在其上扩展多轨编辑、导出和交付工作流。
